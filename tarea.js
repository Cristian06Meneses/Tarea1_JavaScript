...
function tienenMismaLongitud(str1, str2) {
  ...
  const nuevoUsuario = (nombre, email, password) => {
    ...
    const usuarios ={nombre: nombre, email: email, password: password};
  ...
  const pasarUsuarioAPremium = usuarios => {
    ...
    usuarios.forEach((valor) => valor.esPremium = true);
    return usuarios;
  }
  ...
  const sumarLikesDeUsuario = usuario => {
    ...
    usuario.posts.forEach((valor) => suma+= valor.likes);
...
function filter(array) {
  ...
  const nuevoArray = array.filter((valor) => valor.startsWith('a'));
